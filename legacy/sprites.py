...
#                self.draw_sprite_grid(dirty_sprite, surface)
  ...
  def get_dirty_size(self):
    ...
    size = 0
    for value in self._layers_to_dirty.values():
      size += len(value)
    return size
  ...
  def get_flat_sprites_at(self, target_rect):
    ...
    if target_sprites is not None:
      for layers in target_sprites:
        sprites.update([i for sl in layers.values() for i in sl])
    return sprites
...
class InteractionEntity(SingleEntity):
  ...
  def _get_north_sprite(self):
    ...
    spr = []  # Sprites
    spr.extend([i for sl in g._sprite_matrix[y][x].values() for i in sl])
    for sprite in spr:
